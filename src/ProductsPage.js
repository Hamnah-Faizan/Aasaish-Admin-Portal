...
import {
  ...
  Paper, Typography, IconButton, Button, Modal, Box,
...
function ProductsPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setOpen(false);
      showSuccessSnackbar(isEditing ? 'Product Updated' : 'Product Added');
  ...
  const handleCollectionSubmit = async (event) => {
    ...
    try {
      const response = isEditingCollection ?
        await axios.put(`${BASE_URL}/Collection/updatecollections/${currentCollection._id}`, currentCollection) :
        await axios.post(`${BASE_URL}/Collection/createcollection`, currentCollection);
  ...
  const handleBulkImportSubmit = async (event) => {
    ...
    try {
      ...
      setOpenBulkImport(false);
      fetchProducts();
      showSuccessSnackbar('Bulk Import Successful');
